...
$res = mysql_query('select *, count(tweets.id) as tweet_count, facebook.total_count as fb_total, facebook.share_count as fb_share, facebook.like_count as fb_like, facebook.comment_count as fb_comment from meta left join meta_artikel on meta_artikel.meta_id = meta.id left join artikelen on artikelen.id = meta_artikel.art_id left outer join tweets on tweets.art_id = artikelen.id left outer join facebook on facebook.art_id = artikelen.id where waarde like "%'.$search_string.'%" and NOT artikelen.id IS NULL group by artikelen.id ');
